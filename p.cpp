...
{ //{{{
	...
	vector<double> vec_udist;	// stores distance between u and v when they are an UP, in multiples of r
	...
	{	
		...
		*/
		...
		if (u >= 0) {	
			...
			vec_udist.push_back(distance(u, v, 1, 1)/r);
	...
	printf("Areas of intersection of UPs:\n");
	printf("uarea <- c(\n#{{{\n");
	...
	/*	
	printf("Internodal distances of UPs:\n");
	printf("udist <- c(\n");
	for (vector<double>::const_iterator iter = vec_udist.begin(); iter != vec_udist.end(); iter++) {
		if (iter + 1 == vec_udist.end())
			printf("%f)\n", *iter);
		else
			printf("%f,\n", *iter);
	}	
	*/
	...
	/*
	printf("Angles of UPs:\n");
	printf("uangle <- c(\n");
	for (vector<double>::const_iterator iter = vec_udist.begin(); iter != vec_udist.end(); iter++) {		
		if (iter + 1 == vec_udist.end())			
			printf("%f)\n", 2*acos((*iter)/2));	// *iter has already been divided by r
		else
			printf("%f,\n", 2*acos((*iter)/2));
	}
	*/
